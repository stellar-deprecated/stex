...
Initializer.add('startup', 'stex.errors', ['stex.initialize-app', 'stex.cls', 'stex.sentry'], function(stex) {
  ...
  if(stex.sentry) {
    stex.app.use(raven.middleware.express(stex.sentry));
  }
  ...
  stex.reportError = function (err) {
    ...
    if (stex.sentry) {
      // put requestId on err
      err.requestId = stex.cls.get("requestId");
...
function logError(err, req, res, next) {
  ...
  log.error("error: " + plainError.message, {
    trace: plainError.stack
  });
